...
int main(int argc, char **argv) {
  ...
  std::unique_ptr<DCTranslator> DT(
    new DCTranslator(*unwrap(LLVMGetGlobalContext()), DL,
...
//        pm->add(new TailCallPass(functionStarts));
    ...
    if (!NoPrint) {
        std::error_code EC;
        sys::fs::OpenFlags OpenFlags = llvm::sys::fs::OF_None;
        if (!Binary)
            OpenFlags |= llvm::sys::fs::OF_Text;
        std::unique_ptr<ToolOutputFile> FDOut = std::make_unique<ToolOutputFile>(OutputFilename, EC,
                                                         OpenFlags);
        if (EC) {
            errs() << EC.message() << '\n';
            return -1;
