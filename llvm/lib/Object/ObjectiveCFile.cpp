...
void ObjectiveCFile::resolveMethods() {
    for(section_iterator S_it = MachO->section_begin(); S_it != MachO->section_end(); ++S_it){
        StringRef SectionName =  *(S_it->getName());
        if (SectionName == "__objc_classlist") {
            ObjcClasslistAddress = S_it->getAddress();
            StringRef ObjcClasslistContent = *(S_it->getContents());
            ...
            StringRef ObjcDataContent = *(S_it->getContents());
            ...
            StringRef DataContent = *(S_it->getContents());
            ...
            StringRef ObjcConstContent = *(S_it->getContents());
            ...
            StringRef ObjcClassnamesContent = *(S_it->getContents());
            ...
            StringRef cStringsContent = *(S_it->getContents());
            ...
            StringRef ObjcMethodnamesContent = *(S_it->getContents());
            ...
            StringRef ObjcCatlistContent = *(S_it->getContents());
    ...
    for (unsigned ClasslistIdx = 0; ClasslistIdx < ObjcClasslistData.size(); ClasslistIdx += sizeof(uint64_t)) {
        ...
        LLVM_DEBUG(errs() << "[+] ObjcDataAddress address: 0x" << utohexstr(ObjcDataAddress) << "\n");
        LLVM_DEBUG(errs() << "[+] ClassData ISA address: 0x" << utohexstr(ClassData->ISA) << "\n");
        ...
        LLVM_DEBUG(errs() << "[+] ClassData->Data address: " << utohexstr(ClassData->Data) << "\tObjcConstAddress: " << utohexstr(ObjcConstAddress) << "\tisSwift: "
        ...
        if (ClassData->ISA) {
            ...
            if (isSwiftClass) {
                ...
                LLVM_DEBUG(errs() << "[+]ISAClassInfo->BaseMethods address: "<< utohexstr(ISAClassInfo->BaseMethods) << "\n");
                LLVM_DEBUG(errs() << "[+]ISAClassInfo->Name: 0x" << utohexstr(ISAClassInfo->Name) << "\n");
                ...
                LLVM_DEBUG(dbgs() << "ISA: " << utohexstr(ISAData->ISA) << "\n");
    ...
    if (ObjcCatlistData.size()) {
        for (unsigned CatlistIdx = 0; CatlistIdx < ObjcCatlistData.size(); CatlistIdx += sizeof(uint64_t)) {
            ...
            if (catInfo->Class) {
                ...
                //try to fix ClassInfo address error(isSwift)
                LLVM_DEBUG(errs() << "[+] Castlist ClassData->Data address: " << utohexstr(ClassData->Data) << "\n");
...
void ObjectiveCFile::resolveMethods(ObjcCatInfoStruct_t *CatInfo, bool ClassMethods, uint64_t CatInfoAddress, ObjcClassInfoStruct_t *ClassInfo, bool isSwiftClass) {
    ...
    LLVM_DEBUG(dbgs() << "[+]CatInfo->Class address: 0x" << utohexstr(CatInfo->Class));
    if (CatInfo->Class) {
        assert(ClassInfo && ClassInfo->Name);
        LLVM_DEBUG(dbgs() << "[+]CatInfo->Name address: 0x" << utohexstr(CatInfo->Name));
        StringRef ClassName;
        if(isSwiftClass) {
            ClassName = getClassName(cStringsData, cStringsAddress, ClassInfo->Name, true);
        }
