...
void MCObjectDisassembler::disassembleFunctionAt(
  ...
  LLVM_DEBUG(dbgs() << "Starting CFG at " << utohexstr(BBBeginAddr) << "\n");
  ...
  for (size_t wi = 0; wi < Worklist.size(); ++wi) {
    ...
    LLVM_DEBUG(dbgs() << "Looking for block at " << utohexstr(BeginAddr) << "\n");
    ...
    if (BeforeIt != BBInfos.end() && BeforeIt->first < BeginAddr) {
      ...
      LLVM_DEBUG(dbgs() << "Found block at " << utohexstr(BeforeBB.BeginAddr)
      ...
      LLVM_DEBUG(dbgs() << "No existing block found, starting disassembly from "
      ...
      for (uint64_t Addr = BeginAddr; Addr < EndAddr; Addr += InstSize) {
        ...
        if (Dis.getInstruction(Inst, InstSize,
          ...
          LLVM_DEBUG(dbgs() << "Failed disassembly at " << utohexstr(Addr) << "!\n");
          break;
        }
        ...
        if (MIA.evaluateBranch(Inst, Addr, InstSize, BranchTarget)) {
          LLVM_DEBUG(dbgs() << "Found branch to " << utohexstr(BranchTarget)
                       << "!\n");
          if (MIA.isCall(Inst)) {
...
//                  CallTargets.push_back(BranchTarget);
        ...
        if (MIA.isTerminator(Inst) || lastInst) {
          LLVM_DEBUG(dbgs() << "Found terminator!\n");
