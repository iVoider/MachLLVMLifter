...
MCDisassembler::DecodeStatus MCCachingDisassembler::getInstruction(
    MCInst &Inst, uint64_t &InstSize, ArrayRef<uint8_t> Bytes, uint64_t Addr, raw_ostream &cStream) const {
  ...
  DecodeStatus S =
      Impl.getInstruction(Inst, InstSize, Bytes, Addr, cStream);
...
void MCCachingDisassembler::uniqueTempInstructions() const {
  ...
  LLVM_DEBUG(dbgs() << " Trying to unique \n");
  LLVM_DEBUG(dbgs() << " Uniqued " << NumUniquedInsts << " and translated "
               << NumTranslatedInsts << " \n");
  ...
  LLVM_DEBUG(dbgs() << " Cached " << CachedInsts.size() <<  " \n");
