...
VMAddrSlide(VMAddrSlide) {
    ...
    for (const SectionRef &Section : MOOF.sections()) {
        ...
        if (Name == "__mod_init_func") {
            LLVM_DEBUG(dbgs() << "Found __mod_init_func section!\n");
            ModInitContents = *(Section.getContents());
        } else if (Name == "__mod_exit_func") {
            LLVM_DEBUG(dbgs() << "Found __mod_exit_func section!\n");
            ModExitContents = *(Section.getContents());
        }
    }
}
...
tryAddingSymbolicOperand(MCInst &MI, raw_ostream &cStream,
                         ...
                         uint64_t Offset, uint64_t OpSize, uint64_t InstSize) {
    if (IsBranch) {
