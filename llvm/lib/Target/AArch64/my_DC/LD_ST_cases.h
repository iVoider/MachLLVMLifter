      ...
      case AArch64::LD4Fourv8h:
      {
        ...
        if (regNo >= AArch64::B0 && regNo <= AArch64::B31) {
          loadType = IntegerType::get(*unwrap(LLVMGetGlobalContext()), 8);
        } else if (regNo >= AArch64::H0 && regNo <= AArch64::H31) {
          loadType = IntegerType::get(*unwrap(LLVMGetGlobalContext()), 16);
        } else if (regNo >= AArch64::S0 && regNo <= AArch64::S31) {
          loadType = IntegerType::get(*unwrap(LLVMGetGlobalContext()), 32);
        } else if (regNo >= AArch64::D0 && regNo <= AArch64::D31) {
          loadType = IntegerType::get(*unwrap(LLVMGetGlobalContext()), 64);
        } else if (regNo >= AArch64::Q0 && regNo <= AArch64::Q31) {
          loadType = IntegerType::get(*unwrap(LLVMGetGlobalContext()), 128);
        } else if (regNo >= AArch64::Q0_Q1 && regNo <= AArch64::Q31_Q0) {
          loadType = IntegerType::get(*unwrap(LLVMGetGlobalContext()), 256);
        } else if (regNo >= AArch64::Q0_Q1_Q2 && regNo <= AArch64::Q31_Q0_Q1) {
          loadType = IntegerType::get(*unwrap(LLVMGetGlobalContext()), 384);
        } else if (regNo >= AArch64::Q0_Q1_Q2_Q3 && regNo <= AArch64::Q31_Q0_Q1_Q2) {
          loadType = IntegerType::get(*unwrap(LLVMGetGlobalContext()), 512);
        } else if (regNo >= AArch64::D0_D1 && regNo <= AArch64::D31_D0) {
          loadType = IntegerType::get(*unwrap(LLVMGetGlobalContext()), 128);
        } else if (regNo >= AArch64::D0_D1_D2 && regNo <= AArch64::D31_D0_D1) {
          loadType = IntegerType::get(*unwrap(LLVMGetGlobalContext()), 192);
      ...
      case AArch64::ST4i8:
      {
        ...
        switch (Opcode) {
          ...
          case AArch64::ST1i16:
            numVectors = 1;
            numElements = 8;
            elementType = IntegerType::get(*unwrap(LLVMGetGlobalContext()), 16);
            break;
          case AArch64::ST1i32:
            numVectors = 1;
            numElements = 4;
            elementType = IntegerType::get(*unwrap(LLVMGetGlobalContext()), 32);
            break;
          case AArch64::ST1i64:
            numVectors = 1;
            numElements = 2;
            elementType = IntegerType::get(*unwrap(LLVMGetGlobalContext()), 64);
            break;
          case AArch64::ST1i8:
            numVectors = 1;
            numElements = 16;
            elementType = IntegerType::get(*unwrap(LLVMGetGlobalContext()), 8);
            break;
          case AArch64::ST2i16:
            numVectors = 2;
            numElements = 8;
        ...
        dst = Builder->CreateBitCast(dst, IntegerType::get(*unwrap(LLVMGetGlobalContext()), elementType->getScalarSizeInBits() * numVectors));
      ...
      case AArch64::LD4Rv8h:
      {
        ...
        dstVec = Builder->CreateBitCast(dstVec, IntegerType::get(*unwrap(LLVMGetGlobalContext()), numVectors * numElements * elementType->getScalarSizeInBits()));
