...
Function *DCTranslator::translateRecursivelyAt(uint64_t Addr) {
  ...
  for (size_t i = 0; i < WorkList.size(); ++i) {
    ...
    LLVM_DEBUG(dbgs() << "Translating function at " << utohexstr(Addr) << "\n");
    ...
    if (MCFN->empty()) {
      ...
      LLVM_DEBUG(dbgs() << "Found external function: " << ExtFnName << "\n");
...
void DCTranslator::translateFunction(
  ...
  for (auto &BB : *MCFN) {
    ...
    LLVM_DEBUG(dbgs() << "Translating basic block starting at 0x"
